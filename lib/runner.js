...
var Runner = function (app, args, stderr) {
  ...
  this.stack_dirname_ = path.normalize(path.resolve(__dirname, '..'));
...
Runner.prototype.path = function (filename) {
  ...
  if (filename[0] === '/') {
    filename = './' + path.relative(this.project_dirname_, filename);
  }
...
Runner.prototype.getTask_ = function (task_id) {
  ...
  var task = {};
  task.id = task_id;
  task.filename = path.normalize(this.path(task_desc.filename));
  task.shadow = task_desc.shadow;

  var stack_to_project = path.relative(this.stack_dirname_, this.project_dirname_);
  stack_to_project = path.normalize(stack_to_project);

  var requiredFilename = task.filename;

  if(path.resolve(task.filename) !== task.filename) {
    requiredFilename = path.join(this.stack_dirname_, stack_to_project, task.filename);
  }

  var run = require(requiredFilename);
